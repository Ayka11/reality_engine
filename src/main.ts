...
/**
 * Visible scientific diagnostics for the Infinity Scale runtime.
 *
 * The panel is deliberately created here instead of changing index.html,
 * so the existing UI layout remains untouched.
 *
 * All displayed field values come from the real SparseVoxelGrid.
 * AMR values come from the real Infinity Scale residency records.
 */
let _latestInfinityDiagnostics = {
  gradientRMS: 0,
  gradientMax: 0,
  fieldMin: 0,
  fieldMax: 0,
...
try {
  _latestInfinityDiagnostics = _computeInfinityScaleDiagnostics();
} catch (error) {
  // _infinityDiagnosticsError =
    error instanceof Error ? error.message : String(error);

  console.error(
    "[Reality Engine] Infinity Scale diagnostics failed:",
    error,
  );
}

function _ensureInfinityDiagnosticsPanel(): HTMLElement {
  const existing = document.getElementById('infinityDiagnosticsPanel');
  if (existing) return existing;

  const panel = document.createElement('section');
  panel.id = 'infinityDiagnosticsPanel';

  panel.style.position = 'fixed';
  panel.style.right = '8px';
  panel.style.top = '315px';
  panel.style.width = '210px';
  panel.style.zIndex = '10000';
  panel.style.padding = '10px 11px';
  panel.style.boxSizing = 'border-box';
  panel.style.border = '1px solid rgba(124,159,255,0.28)';
  panel.style.borderRadius = '7px';
  panel.style.background = 'rgba(7,8,18,0.94)';
...
// Create the diagnostics panel immediately after application startup.
// The render loop below only updates its values.
_ensureInfinityDiagnosticsPanel();

function _fmtInfinityValue(value: number, digits = 4): string {
  return Number.isFinite(value) ? value.toFixed(digits) : '—';
}

function _updateInfinityDiagnosticsPanel(): void {
  const panel = _ensureInfinityDiagnosticsPanel();
  ...
  for (const record of records) {
    if (record.amr === 0) amr0++;
    else if (record.amr === 1) amr1++;
    else if (record.amr >= 2) amr2++;
  }

  panel.innerHTML = `
    <div style="font-weight:700;color:#d7ddff;letter-spacing:.08em;margin-bottom:7px;">
      INFINITY SCALE
    </div>

    <div style="color:#6f7b99;margin-bottom:4px;">OBSERVER</div>
    <div style="display:flex;justify-content:space-between;">
      <span>Position</span><span style="color:#d7ddff;">${observer.x}, ${observer.y}, ${observer.z}</span>
    </div>

    <div style="color:#6f7b99;margin:7px 0 4px;">RESIDENCY</div>
    <div style="display:flex;justify-content:space-between;">
      <span>Active chunks</span><span style="color:#8eceab;">${sim.grid.activeChunkCount}</span>
    </div>
    <div style="display:flex;justify-content:space-between;">
      <span>Known chunks</span><span>${sim.grid.chunks.size}</span>
    </div>
    <div style="display:flex;justify-content:space-between;">
    ...
    <div style="display:flex;justify-content:space-between;">
      <span>Min</span><span style="color:#7c9fff;">${_fmtInfinityValue(d.fieldMin, 2)}</span>
    </div>
    <div style="display:flex;justify-content:space-between;">
      <span>Max</span><span style="color:#7c9fff;">${_fmtInfinityValue(d.fieldMax, 2)}</span>
    </div>
    <div style="display:flex;justify-content:space-between;">
      <span>Gradient RMS</span><span style="color:#ef8f3f;">${_fmtInfinityValue(d.gradientRMS, 6)}</span>
    </div>
    <div style="display:flex;justify-content:space-between;">
      <span>Gradient Max</span><span style="color:#ef8f3f;">${_fmtInfinityValue(d.gradientMax, 6)}</span>
    </div>
    <div style="display:flex;justify-content:space-between;">
      <span>Valid samples</span><span>${d.validSamples.toLocaleString()}</span>
    </div>

    <div style="color:#6f7b99;margin:7px 0 4px;">AMR LEVELS</div>
    <div style="display:flex;justify-content:space-between;">
      <span>L0</span><span>${amr0}</span>
    </div>
    <div style="display:flex;justify-content:space-between;">
      <span>L1</span><span>${amr1}</span>
    </div>
    <div style="display:flex;justify-content:space-between;">
      <span>L2+</span><span>${amr2}</span>
    </div>
  `;
}
...
async function loop(ts: number) {
  ...
  // Scientific Infinity Scale diagnostics.
  _updateInfinityDiagnosticsPanel();
