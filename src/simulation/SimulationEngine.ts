...
export class SimulationEngine {
  ...
  async step(dt: number, nSteps = 1): Promise<void> {
    ...
    if (
      ...
      // Reconcile dense level-0 solver output into hierarchical state before
      // staging cross-LOD transfers, avoiding stale pre-step boundary values.
      this.syncInfinityScaleDenseState(frameContext);
