...
export class InfinityScalePredictionEngine {
  ...
  observe(regionId: string, epoch: number, error: number): InfinityScalePredictiveState {
    ...
    const state = this.states.get(regionId) ?? {
      regionId,
      samples: [],
      lastPredictionEpoch: -1,
    };

    const bounded = clamp01(error);
    const previous = state.samples[state.samples.length - 1];
    if (previous && epoch <= previous.epoch) {
      throw new Error(`Prediction samples must be strictly increasing for ${regionId}`);
    }

    state.samples.push({ epoch, error: bounded });
    if (state.samples.length > this.config.historyLength) {
      state.samples.splice(0, state.samples.length - this.config.historyLength);
    }
  ...
  predict(regionId: string, horizon = 1): InfinityScalePredictionResult {
    ...
    if (!state || state.samples.length < this.config.minSamples) {
      const result = {
        predictedError: state?.samples.at(-1)?.error ?? 0,
        trend: 0,
        acceleration: 0,
        horizon,
        confidence: 0,
    ...
    const trend = (last.error - previous.error) / Math.max(1, last.epoch - previous.epoch);

    let acceleration = 0;
    if (samples.length >= 3) {
      const before = samples[samples.length - 3];
      const previousTrend =
        (previous.error - before.error) / Math.max(1, previous.epoch - before.epoch);
      acceleration = trend - previousTrend;
    }

    const predictedError = clamp01(last.error + horizon * trend + 0.5 * horizon * horizon * acceleration);

    const residuals: number[] = [];
    for (let i = 1; i < samples.length; i++) {
      const dt = samples[i].epoch - samples[i - 1].epoch;
      const predicted = clamp01(
        samples[i - 1].error +
        trend * dt +
        0.5 * acceleration * dt * dt,
      );
      residuals.push(Math.abs(samples[i].error - predicted));
    }
    ...
    const residualConfidence = Math.max(
      0,
      1 - residual / this.config.confidenceResidualScale,
    );
