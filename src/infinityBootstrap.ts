...
export function bootstrapInfiniteWorld() {
  ...
  ;(installScientificWorkspace as () => void)()
  ...
  if (wrap) {
    ...
    const renderLibraryShelf = () => {
      ...
      const related = selected ? worldRuleGraph.outgoing(selected.id) : []
    ...
    const render = () => {
      ...
      document.getElementById('worldLibraryCategory')?.addEventListener('change', (event) => {
        libraryCategory = (event.target as HTMLSelectElement).value as typeof libraryCategory
        render()
      })
