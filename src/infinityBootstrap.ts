...
import { buildCivilizationExperimentMatrix, runCivilizationInfinityBatch, analyzeCrossDimensionGeneralization, summarizeReplicationSnapshots, createInfinityScientificReport, serializeInfinityScientificReport, type CivilizationExperimentMatrix } from './infinity'
...
export function bootstrapInfiniteWorld() {
  ...
  ;(window as any).settlementGrowthTick = (state: any, delta = 1) => {
    ...
    if (result.nextTier) {
      const materialized = world.materializeSettlementTier(result.nextTier)
  ...
  ;(window as any).worldSerializeInfinityScientificReport = (report: any) => serializeInfinityScientificReport(report)
  ...
  ;(window as any).worldExperimentClaimGraph = (experimentId: string, scenarios: any[], ticks = 10, delta = 1) => civilizationRuntime.buildClaimGraph((civilizationRuntime.experiment(experimentId) ?? civilizationRuntime.runExperiment(experimentId, scenarios, ticks, delta)).outcomes)
  ;(window as any).worldComparativeExperimentClaimGraph = (experimentId: string, scenarios: any[], ticks = 10, delta = 1) => civilizationRuntime.buildComparativeClaimGraph((civilizationRuntime.experiment(experimentId) ?? civilizationRuntime.runExperiment(experimentId, scenarios, ticks, delta)).outcomes)
  ...
  ;(window as any).worldCivilizationBranchCausalEvents = (branchId: string) => civilizationRuntime.branchCausalEvents(branchId)
  ...
  ;(window as any).worldEnvironmentResolve = (environment: any, category?: string) =>
    worldEnvironmentResolver.resolve(environment, category as any)
  ;(window as any).worldEnvironmentRelated = (environment: any, seedIds: string[] = []) =>
