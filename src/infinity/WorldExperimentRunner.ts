...
export function runWorldExperiment(
  ...
  if (
    protocol.field.providerId !== provider.id ||
    protocol.field.providerVersion !== provider.version
  ) {
    throw new Error(
      `World experiment provider ${protocol.field.providerId}@${protocol.field.providerVersion} is not available in this runner`,
    )
  }
  ...
  const runner = options.runner ?? new ExperimentRunner()
