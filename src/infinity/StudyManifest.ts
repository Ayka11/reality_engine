...
export type StudyManifest = {
  ...
  plans:Array<{experimentId:string;protocol:ExperimentSnapshot['protocol']}>
...
export function createStudyManifest(input:Omit<StudyManifest,'manifestVersion'|'createdAt'>):StudyManifest {
  return {manifestVersion:'study-manifest-v1',createdAt:Date.now(),...input}
}
...
export function studyManifestSummary(manifest:StudyManifest) {
  ...
  return {studyId:manifest.studyId,manifestVersion:manifest.manifestVersion,experiments:manifest.runs.length,completed,aborted,planned:manifest.plans.length,generalizationDimensions:Object.keys(manifest.generalization),evidenceNodes:manifest.evidence?.nodes.length??0,evidenceEdges:manifest.evidence?.edges.length??0,operationalizedHypotheses:manifest.operationalization?.hypotheses.length??0,analyzedHypotheses:manifest.hypothesisAnalysis?.results.length??0}
}
